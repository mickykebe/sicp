...
;procedure-param-symbols & procedure-param-choices added below
(define (apply procedure arguments env) ;changed
  (cond ((primitive-procedure? procedure)
        ...
        ((compound-procedure? procedure)
         (eval-sequence
          ...
          (extend-environment
           (procedure-param-symbols procedure) ; changed
           (list-of-args arguments (procedure-param-choices procedure PARAM-NON-LAZY-CHOICE) env) ; changed
...
;addition:
(define PARAM-NON-LAZY-CHOICE 'non-lazy)
(define PARAM-LAZY-CHOICE 'lazy)
(define PARAM-LAZY-MEMO-CHOICE 'lazy-memo)

(define (param-non-lazy? choice) (eq? choice PARAM-NON-LAZY-CHOICE))
(define (param-lazy? choice) (eq? choice PARAM-LAZY-CHOICE))
(define (param-lazy-memo? choice) (eq? choice PARAM-LAZY-MEMO-CHOICE))

(define (list-of-args exps choices env)
    (define (arg-list exps choices)
        (if (null? exps) 
            '()
            (let ((exp (first-operand exps))
                  (choice (car choices)))
              (cond ((param-non-lazy? choice) (cons (eval exp env)
                                                    (arg-list (rest-operands exps) (cdr choices))))
                    ((param-lazy? choice) (cons (delay-it PARAM-LAZY-CHOICE exp env)
                                                (arg-list (rest-operands exps) (cdr choices))))
                    ((param-lazy-memo? choice) (cons (delay-it PARAM-LAZY-MEMO-CHOICE exp env)
                                                     (arg-list (rest-operands exps) (cdr choices))))
                    (else (error "Invalid argument choice"))))))
    (if (= (length exps) (length choices))
        (arg-list exps choices)
        (error "Malformed arguments supplied")))
...
(define (delay-it memo-choice exp env) ;changed
  (list 'thunk memo-choice exp env))

(define (thunk? obj)
  (tagged-list? obj 'thunk))

(define (thunk-memo? obj) ;changed
  (eq? (cadr obj) PARAM-LAZY-MEMO-CHOICE))

(define (thunk-no-memo? obj) ;changed
  (eq? (cadr obj) PARAM-LAZY-CHOICE))

(define (thunk-exp thunk) (caddr thunk)) ;changed

(define (thunk-env thunk) (cadddr thunk)) ;changed
...
;-------------4.31---------------------
(define (procedure-param-symbols p) ;added
    (map (lambda (param) 
            (if (pair? param)
                (car param)
                param)) 
...
(define (procedure-param-choices p default-choice) ;added
    (map (lambda (param)
            (if (pair? param)
                (cadr param)
                default-choice))
